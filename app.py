...
from lib.enrich import get_api_key
...
def run_refresh():
    api_key = get_api_key()
    if not api_key:
        st.session_state.last_refresh_result = {"errors": ["No ANTHROPIC_API_KEY set — cannot enrich new items."]}
        return
...
if not UPDATES and page not in ("Profile", "Source Status"):
    ...
    if not get_api_key():
        st.warning("Set `ANTHROPIC_API_KEY` (environment variable or Streamlit secret) and click **🔄 Refresh live data** in the sidebar to pull real sources and run the AI analysis.")
    else:
...
if page == "Overview":
    ...
    if st.button("✨ Generate my weekly report"):
        api_key = get_api_key()
        if not api_key:
            st.error("No ANTHROPIC_API_KEY found. Set it as an environment variable or Streamlit secret.")
        else:
...
Every point must connect specifically to GCP + Data Engineering + AI — never generic filler."""
                    response = client.messages.create(model="claude-sonnet-4-6", max_tokens=1400,
                                                        messages=[{"role": "user", "content": prompt}])
                    text = "".join(b.text for b in response.content if hasattr(b, "text"))
                    st.session_state.weekly_report = text
